...
/**
 * Origin to mint invite links against.
 *
 * Inside the packaged app the page origin is https://localhost, which is
 * useless to a recipient. PUBLIC_ORIGIN overrides it so an invite created on a
 * phone opens the same way as one created in a browser.
 */
function inviteOrigin() {
    const configured = (window.WAYSERA_CONFIG && window.WAYSERA_CONFIG.PUBLIC_ORIGIN) || '';
...
async function createJourney() {
    ...
    const inviteLink = WayseraCrypto.buildInviteLink(inviteOrigin(), code, encodedKey);
...
async function shareCurrentInvite() {
    ...
    const link = WayseraCrypto.buildInviteLink(
        inviteOrigin(), currentRoom.room_id, encodedKey
    );
    shareInvite(link);
}
